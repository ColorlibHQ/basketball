...
// social media
if ( ! function_exists( 'basketball_social' ) ) {
	function basketball_social( $args = array()  ){
		...
		$url = basketball_opt('basketball_social_url');
...
function basketball_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'basketball' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'basketball' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'basketball' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function basketball_set_post_views($postID) {
    $count_key = 'basketball_post_views_count';
...
// blog post categoty 
function basketball_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'basketball' ) );
