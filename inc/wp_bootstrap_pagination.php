...
function basketball_pagination( $args = array() ) {
    ...
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'basketball_pagination_defaults', $defaults )
    );
    ...
    if ( $previous && (1 != $page) )
        $echo .= '<li class="previous pgn__prev"><a href="' . esc_attr( $previous ) . '" title="' . esc_html__( 'previous', 'basketball' ) . '">' . esc_html__( 'Previous', 'basketball' ) . '</a></li>';
    ...
    if ($next && ($count != $page) )
        $echo .= '<li class="next pgn__next"><a href="' . esc_attr( $next ) . '" title="' . esc_html__( 'next', 'basketball') . '">' . esc_html__( 'Next', 'basketball') . '</a></li>';
