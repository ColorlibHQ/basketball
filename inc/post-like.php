 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_basketball_process_simple_like', 'basketball_process_simple_like' );
add_action( 'wp_ajax_basketball_process_simple_like', 'basketball_process_simple_like' );
function basketball_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'basketball' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'basketball' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'basketball' );
		$icon = $icon_empty;
	}
...
function get_liked_icon() {
	/* If already using Font Awesome with your theme, replace svg with: <i class="fa-solid fa-heart"></i> */
	$icon = '<i class="fa-solid fa-heart" aria-hidden="true"></i> ';
	return $icon;
} // get_liked_icon()
...
function get_unliked_icon() {
	/* If already using Font Awesome with your theme, replace svg with: <i class="fa-regular fa-heart"></i> */
	$icon = '<i class="fa-regular fa-heart" aria-hidden="true"></i> ';
	return $icon;
} // get_unliked_icon()
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'basketball' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'basketball' );
	} else {
		$number = $like_text . esc_html__( ' Like', 'basketball' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'basketball_show_user_likes' );
add_action( 'edit_user_profile', 'basketball_show_user_likes' );
function basketball_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'basketball' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'basketball' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
